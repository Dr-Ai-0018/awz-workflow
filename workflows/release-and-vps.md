...
- 打包入口需要 Python 3 标准库执行确定性 gzip helper；项目初始化入口仍保持无 Python 前置。
- release 包必须直接来自 committed `HEAD` 的 Git tracked blobs，并在打包子进程显式禁用 `core.autocrlf`，不复制或转换 working-tree 字节；确定性 gzip helper 不依赖平台 zlib 的压缩输出，同一 commit 在 Windows/POSIX 生成的整个 `.tar.gz`、路径和逐文件 SHA-256 都必须相同。
