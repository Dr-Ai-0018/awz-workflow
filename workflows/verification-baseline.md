...
递归删除、批量覆盖和根据变量计算目标的清理属于高风险操作。用户确认只能授权明确范围，不能替代路径校验；安全判断失败时宁可保留临时产物并报告，也不能扩大删除范围。

- 不把 `HOME`、`USERPROFILE`、`CODEX_HOME` 或其他系统环境变量改作任务沙盒变量；使用任务专属变量，并优先指向项目内 `temp/`。
- 删除前把允许根目录和目标解析为绝对规范路径，拒绝空值、文件系统根、用户目录、仓库根、允许根本身、父目录逃逸和符号链接 / reparse point 跳转。
- 测试清理同时校验目录身份，例如目标必须位于 `temp/` 下且顶层目录匹配 `smoke-*`；不要只凭变量名或字符串包含关系判断安全。
- 后台执行必须使用显式函数或括号分组；不要在同一复杂命令链中混合环境初始化、`&`、进程等待和递归清理，也不要依赖肉眼判断 `&&`、`;`、`&` 的结合范围。
- 清理集中放在 `trap` / `finally`，并通过共享 guarded cleanup helper 执行；禁止在多个 smoke 脚本中复制未经负向测试的 `rm -rf` 或 `Remove-Item -Recurse`。
- helper 至少要有拒绝空路径、文件系统根、用户目录、仓库根、允许根、越界路径和错误目录身份的负向测试，并有一个受控目录可成功删除的正向测试。
- guarded cleanup helper 或调用方变化时，先运行 `smoke-safety.ps1` / `smoke-safety.sh`，再运行实际受影响的 smoke；安全 smoke 不能替代调用方闭环。
...
验证不是“改完就跑全套”，而是用最小成本获得足够证据。问题优先级 `P0/P1/P2/P3` 描述影响严重程度；验证级别 `V0/V1/V2/V3` 单独描述验证强度，不要混用。

| 级别 | 典型改动 | 默认验证 |
| --- | --- | --- |
| `V0` | 文案、注释、格式、无行为配置 | diff review、语法或静态检查；通常不跑测试 |
| `V1` | 单函数、单模块局部逻辑 | 相关 test file、最小 smoke 或入口检查 |
| `V2` | 跨模块逻辑、API、schema、数据流 | 受影响模块验证 + 必要的 integration/contract check |
| `V3` | 公共核心、安全、并发、持久化、依赖、release | 明确范围的 Full Check；必要时再做 E2E/UI/build |

### 标准开发节奏

每个逻辑切片按同一条主线推进：

1. **范围基线**：读取规则、status、相关代码、Git 状态和已有入口，写清本轮不包含什么。
2. **风险分级**：选择最低充分的 `V0–V3`，列出必须覆盖的风险和允许的验证成本。
3. **单切片实现**：只完成当前范围，保持改动可 review，不用额外重构制造新的验证面。
4. **Diff review**：检查行为、回归、边界和安全风险，确认验证计划仍与实际 diff 相符。
5. **最小验证**：按下方周期规则执行一次 focused 或 gate-defined check，记录证据。
6. **停止/交接**：证据达到验收条件就停止；未覆盖风险、阻塞和下一步进入交接，不用新一轮检查代替判断。

### 验证周期

1. 先记录验证目标、变更范围、代码树身份（`HEAD` 加相关 diff）和可用入口。
2. 选择能覆盖当前风险的最低 `V` 级别；V0–V2 默认只执行一轮 focused verification，V3 按明确 gate 定义的检查范围执行一轮。
3. 同一代码树 + 同一命令 + 同一环境已有通过证据时，禁止重复执行；review、换 Agent 或“再确认一次”不构成新理由。
4. focused 通过后直接收口。只有测试失败、相关代码继续变化、出现新风险，或进入明确的合并/release gate，才能开始下一轮。
5. 每轮结束记录命令、结果、覆盖范围、未覆盖风险和是否允许升级；证据足够时停止，不为了延长检查而继续扫描。

### 升级与成本边界

- Full suite、E2E、UI test、benchmark 和长时间 build 必须满足至少一个条件：用户明确要求或项目 gate 明确要求；V3 风险只提高评估等级，不单独授予昂贵验证授权。
- 如果验证预计昂贵且不是项目硬 gate，先报告成本和覆盖目标，等待用户确认或转交 CI；不能把“最终步骤”当作自动授权。
- 测试命令失败时，先区分代码失败与环境失败。环境失败默认只允许一次针对性诊断和一次合理 fallback；仍失败就报告 blocker，不无限寻找替代解释器或重复命令。
- 变更后只重跑受影响的验证；格式化或文档 review 没有改变相关行为时，不重跑同一测试。
