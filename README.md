...
Reference root 配置、Reference `add`、项目 `map/unmap/context`、fast-forward `update`、`unregister`、`trash` 与 `restore` 已接入控制中心：先执行 DryRun，再使用 planHash 绑定 apply，并留下 transaction 记录；永久 `purge` 尚未开放。自动化环境可以传入完整 init 参数和 `-Yes` / `--yes`，但不能跳过预览与底层安全检查。详细契约见 [TUI 与 CLI 工作流](workflows/tui.md)。
