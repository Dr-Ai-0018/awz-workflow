...
Reference root 配置已接入控制中心：先执行 DryRun，再使用 planHash 绑定 apply，并留下 transaction 记录；登记、映射、context、更新、trash/restore 等其他写入生命周期仍建议使用 CLI，直到各自的预览与恢复界面完成。自动化环境可以传入完整 init 参数和 `-Yes` / `--yes`，但不能跳过预览与底层安全检查。详细契约见 [TUI 与 CLI 工作流](workflows/tui.md)。
